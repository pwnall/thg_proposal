...
complex ways. \S \ref{sec:overview} reviews these features and mentions attacks
that have compromised the software running at all privilege levels, forcing us
to conclude that we cannot rely on any software running on the infrastructure.
...
now in the CPU, so it is part of the TCB, and we can rely on it to protect a
range of physical memory from malicious devices connected to the system bus, by
rejecting DMA transfers.
