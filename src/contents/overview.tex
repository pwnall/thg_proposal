...
Our system uses a scheme for hiding memory access patterns that relies on some
details of the cache implementation on commodity CPU processors. To remain
future-proof, the loader uses the CPUID instruction \cite{intel2013manual} to
query the CPU's cache configuration and aborts when faced with a configuration
that doesn't match our design. This section describes the aspects that we rely
on.
